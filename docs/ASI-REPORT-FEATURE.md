...
EXOCHAIN is a constitutional trust fabric: 20 Rust workspace packages, 115663 lines of Rust under `crates/`, 2,861 listed tests, and a formal proof chain demonstrating its intended governance properties.
...
EXOCHAIN is that judiciary for AI systems. Its five constitutional properties, backed by formal proofs and 957 automated tests, provide the structural guarantee that:
...
These aren't aspirational. They're enforced at the type level, the runtime level, and the cryptographic level. They are as immutable as the code that implements them — and that code is verified by formal proofs, automated tests, and a constitutional governance process that governs its own evolution.
