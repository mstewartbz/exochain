...
pub struct AuditEntry {
    pub id: Uuid, pub timestamp: Timestamp, pub actor: Did,
    pub action: String, pub result: String, pub evidence_hash: [u8; 32],
...
impl AuditLog {
    ...
    pub fn head_hash(&self) -> [u8; 32] {
        self.entries.last().map(|e| hash_entry(e)).unwrap_or([0u8; 32])
    }
...
pub fn append(log: &mut AuditLog, entry: AuditEntry) -> Result<(), GovernanceError> {
    if entry.chain_hash != log.head_hash() {
        return Err(GovernanceError::AuditChainBroken { index: log.entries.len() });
    }
...
pub fn verify_chain(log: &AuditLog) -> Result<(), GovernanceError> {
    ...
    for (i, entry) in log.entries.iter().enumerate() {
        if entry.chain_hash != prev { return Err(GovernanceError::AuditChainBroken { index: i }); }
...
#[must_use]
pub fn create_entry(log: &AuditLog, actor: Did, action: String, result: String, evidence_hash: [u8; 32]) -> AuditEntry {
    AuditEntry {
        id: Uuid::new_v4(), timestamp: Timestamp::now_utc(), actor, action, result,
        evidence_hash, chain_hash: log.head_hash(),
    }
}
...
mod tests {
    ...
    #[test] fn empty_log_verifies() { assert!(verify_chain(&AuditLog::new()).is_ok()); }
    #[test] fn append_single() {
        let mut log = AuditLog::new();
        make_and_append(&mut log, "a1");
        assert_eq!(log.len(), 1); assert!(!log.is_empty());
    }
    #[test] fn chain_of_entries() {
        let mut log = AuditLog::new();
        for i in 0..5 { make_and_append(&mut log, &format!("a{i}")); }
        assert_eq!(log.len(), 5);
        assert!(verify_chain(&log).is_ok());
    }
    #[test] fn tamper_detected() {
        let mut log = AuditLog::new();
        for i in 0..3 { make_and_append(&mut log, &format!("a{i}")); }
    ...
    #[test] fn wrong_chain_hash_rejected() {
        let mut log = AuditLog::new();
        make_and_append(&mut log, "a1");
        let bad = AuditEntry {
            id: Uuid::new_v4(), timestamp: Timestamp::new(2000, 0), actor: did("x"),
            action: "bad".into(), result: "bad".into(), evidence_hash: [0u8; 32], chain_hash: [0xffu8; 32],
        };
        assert!(append(&mut log, bad).is_err());
    }
    #[test] fn head_hash_changes() {
        let mut log = AuditLog::new();
    ...
    #[test] fn deterministic_hash() {
        let e = AuditEntry {
            id: Uuid::nil(), timestamp: Timestamp::new(1000, 0), actor: did("test"),
            action: "test".into(), result: "ok".into(), evidence_hash: [0u8; 32], chain_hash: [0u8; 32],
        };
