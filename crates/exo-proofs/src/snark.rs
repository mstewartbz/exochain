...
pub fn setup(circuit: &dyn Circuit) -> Result<(ProvingKey, VerifyingKey)> {
    ...
    let circuit_hash = compute_circuit_hash(&cs);
...
fn compute_circuit_hash(cs: &ConstraintSystem) -> Hash256 {
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"snark:circuit:");
    hasher.update(&(cs.num_variables() as u64).to_le_bytes());
    hasher.update(&(cs.num_constraints() as u64).to_le_bytes());
    hasher.update(&(cs.num_public_inputs as u64).to_le_bytes());
...
fn compute_c_component(circuit_hash: &Hash256, public_inputs: &[u64], a: &[u8; 32], b: &[u8; 32]) -> [u8; 32] {
    let mut hasher = blake3::Hasher::new();
...
mod tests {
    use super::*;
    use crate::circuit::{allocate, allocate_public, enforce, LinearCombination};
