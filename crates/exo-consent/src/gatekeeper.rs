...
use crate::bailment::{self, Bailment};
use crate::policy::{ActionRequest, ActiveConsent, ConsentDecision, ConsentPolicy, PolicyEngine};
...
impl ConsentGate {
    ...
    pub fn new(policy: ConsentPolicy) -> Self {
        Self { engine: PolicyEngine::new(), policy, bailments: BTreeMap::new(), consents: BTreeMap::new() }
    }

    pub fn register_bailment(&mut self, bailment: Bailment) {
        self.bailments.entry(bailment.bailee_did.as_str().to_owned()).or_default().push(bailment);
    }

    pub fn register_consent(&mut self, actor: &Did, action_type: &str, role: &str, clearance_level: u32, bailment: Bailment) {
        self.consents.entry(actor.as_str().to_owned()).or_default().push(ConsentReg {
            action_type: action_type.into(), role: role.into(), clearance_level, bailment,
        });
    }

    pub fn revoke_by_bailment_id(&mut self, bailment_id: &str) {
        for regs in self.consents.values_mut() { regs.retain(|r| r.bailment.id != bailment_id); }
        for bs in self.bailments.values_mut() { bs.retain(|b| b.id != bailment_id); }
    }

    /// Check consent. Default: DENY.
    pub fn check(&self, actor: &Did, action: &str, now: &Timestamp) -> ConsentDecision {
        let req = ActionRequest { actor: actor.clone(), action_type: action.into() };
        let active: Vec<ActiveConsent> = self.consents.get(actor.as_str()).map(|regs| {
            regs.iter()
    ...
    #[must_use]
    pub fn policy(&self) -> &ConsentPolicy { &self.policy }

    pub fn set_policy(&mut self, policy: ConsentPolicy) { self.policy = policy; }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::bailment::{self, BailmentType};
    use crate::policy::ConsentRequirement;
    ...
    fn strict_policy() -> ConsentPolicy {
        ConsentPolicy {
            id: "strict".into(), name: "strict".into(), deny_by_default: true,
            required_consents: vec![ConsentRequirement { action_type: "read".into(), required_role: "data-owner".into(), min_clearance_level: 1 }],
        }
    }

    fn make_bailment(bailor: &Did, bailee: &Did, bt: BailmentType, exp: Option<Timestamp>) -> Bailment {
    ...
    fn default_deny() {
        ...
        assert!(matches!(g.check(&bob(), "read", &now()), ConsentDecision::Denied { .. }));
    }
    ...
    fn grant_with_valid_consent() {
        ...
        assert!(matches!(g.check(&bob(), "read", &now()), ConsentDecision::Granted { .. }));
    }
    ...
    fn grant_with_future_expiry() {
        ...
        g.register_bailment(b.clone());
        g.register_consent(&bob(), "read", "data-owner", 1, b);
        assert_eq!(g.check(&bob(), "read", &now()), ConsentDecision::Granted { expires: Some(exp) });
    }
    ...
    fn deny_unknown_action() {
        ...
        let b = make_bailment(&alice(), &bob(), BailmentType::Custody, None);
        g.register_consent(&bob(), "read", "data-owner", 1, b);
        assert!(matches!(g.check(&bob(), "write", &now()), ConsentDecision::Denied { .. }));
    }

    #[test]
    fn deny_unknown_actor() {
        let g = ConsentGate::new(strict_policy());
        assert!(matches!(g.check(&charlie(), "read", &now()), ConsentDecision::Denied { .. }));
    }
    ...
    fn set_policy() {
        let mut g = ConsentGate::new(strict_policy());
        let p = ConsentPolicy { id: "perm".into(), name: "perm".into(), required_consents: vec![], deny_by_default: false };
        g.set_policy(p);
        assert_eq!(g.policy().id, "perm");
        assert!(matches!(g.check(&bob(), "anything", &now()), ConsentDecision::Granted { .. }));
    }
    ...
    fn multiple_consents() {
        let mut g = ConsentGate::new(ConsentPolicy {
            ...
            required_consents: vec![
                ConsentRequirement { action_type: "read".into(), required_role: "data-owner".into(), min_clearance_level: 1 },
                ConsentRequirement { action_type: "write".into(), required_role: "admin".into(), min_clearance_level: 2 },
            ],
        });
        ...
        assert!(matches!(g.check(&bob(), "read", &now()), ConsentDecision::Granted { .. }));
        assert!(matches!(g.check(&bob(), "write", &now()), ConsentDecision::Granted { .. }));
    }
