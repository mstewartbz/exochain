...
pub fn check_completeness(case: &EscalationCase) -> CompletenessResult {
    ...
    match case.path {
        EscalationPath::SybilAdjudication => {
            let required = [
                SybilStage::Detection, SybilStage::Triage, SybilStage::Quarantine,
                SybilStage::EvidentaryReview, SybilStage::ClearanceDowngrade,
                SybilStage::Reinstatement, SybilStage::AuditLog,
            ];
        ...
        EscalationPath::Emergency => {
            if !case.stages_completed.contains(&"emergency_activated".to_string()) {
                missing.push("missing stage: emergency_activated".into());
            }
        }
        EscalationPath::Constitutional => {
            if !case.stages_completed.contains(&"constitutional_review".to_string()) {
    ...
    if (case.status == CaseStatus::Resolved || case.status == CaseStatus::Closed) && case.assignee.is_none() {
...
mod tests {
    ...
    #[test] fn sybil_case_complete_after_all_stages() {
        let mut c = escalate(&signal(75), &EscalationPath::SybilAdjudication);
        c.assignee = Some(exo_core::Did::new("did:exo:reviewer").expect("ok"));
        for stage in [SybilStage::Triage, SybilStage::Quarantine, SybilStage::EvidentaryReview,
                      SybilStage::ClearanceDowngrade, SybilStage::Reinstatement, SybilStage::AuditLog] {
            advance_sybil_stage(&mut c, stage).unwrap();
        }
        assert_eq!(check_completeness(&c), CompletenessResult::Complete);
    }

    #[test] fn resolved_without_assignee_incomplete() {
