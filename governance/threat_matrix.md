...
| **T-17** | **DAG DB Governed Memory Writeback Bypass** | A DAG DB REST or MCP caller could try to persist or retrieve graph memory without tenant-scoped consent, actor provenance, or constitutional gate checks. Mitigations in this PR: `/api/v1/dag-db/*` is merged into the gateway router; default gateway/node builds include the `production-db` path; missing Postgres state, tenant authority, or write signatures fail closed instead of fabricating persistence; import/export fail closed with `403 consent_denied` until distinct import/export consent exists; `DagDbGatekeeperService` enforces active bailment/consent, Ed25519 write signatures over canonical payload hashes, and the constructible invariant subset (`ConsentRequired`, `SeparationOfPowers`, `NoSelfGrant`, `HumanOverride`, `KernelImmutability`, `QuorumLegitimate`) before writeback persistence. RLS migration and transaction tenant binding are present; the MCP gateway proxy requires `dagdb-gateway-proxy` plus configured gateway auth. Final activation evidence is tracked in `docs/dagdb/runtime-activation/rollback-canary-observability.md`. | `crates/exo-gatekeeper/src/dagdb_gate.rs`, `crates/exo-gateway/src/dagdb.rs`, `crates/exo-dag-db-postgres/src/postgres/*`, `crates/exo-node/src/mcp/tools/dagdb.rs` | `dagdb_gate` consent/signature/invariant tests; `dagdb_routes_integration_contract`; `dagdb_cross_tenant_denies_every_get_and_post_route`; `writeback_authorizes_against_real_db_consent_and_identity_state`; `configured_gateway_proxies_all_dagdb_mcp_tools_with_auth_and_tenant_scope` | 🟡 Evidence pending |
