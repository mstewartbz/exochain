...
export class BailmentBuilder {
    ...
    async build() {
        ...
        const proposalId = await computeProposalId(this.#bailor, this.#bailee, this.#scope, this.#durationHours);
        return {
            proposalId,
            bailor: this.#bailor,
            bailee: this.#bailee,
 ...
 */
async function computeProposalId(bailor, bailee, scope, durationHours) {
    ...
    const total = bailorB.length + 1 + baileeB.length + 1 + scopeB.length + 1 + durationB.length;
