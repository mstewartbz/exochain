...
const ECONOMY_OBJECT_KINDS: readonly EconomyObjectKind[] = [
  'mission',
...
function validationError(message: string): TransportError {
...
export function assertJsonObject(value: unknown, context: string): JsonObject {
  if (!isRecord(value)) {
    throw validationError(`${context} must be a JSON object`);
  }
  validateJsonValue(value, context, new WeakSet<object>());
...
function requireJsonValue(record: Record<string, unknown>, key: string, context: string): JsonValue {
  const value = record[key];
  validateJsonValue(value, `${context}.${key}`, new WeakSet<object>());
...
function requireEconomyObjectKind(
  ...
  if (!ECONOMY_OBJECT_KINDS.includes(value as EconomyObjectKind)) {
    throw validationError(`${context}.${key} must be a known economy object kind`);
  }
  return value as EconomyObjectKind;
}
...
export function validateHash256(value: unknown, context: string): Hash256 {
...
export function validateDidResponse(value: unknown, context: string): { did: Did } {
...
export function validateHashResponse<K extends string>(
  value: unknown,
  key: K,
  context: string,
  ...
  } as { readonly [P in K]: Hash256 };
}

export function validateQuorumResult(value: unknown, context: string): QuorumResult {
  const record = requireRecord(value, context);
  return {
...
export function validateDecisionState(value: unknown): {
...
export function validateEconomyRecordAnchor(
  value: unknown,
  context: string,
): EconomyRecordAnchor {
  const record = requireRecord(value, context);
  return {
    ...
    previous_anchor_hash: validateHash256(
      record.previous_anchor_hash,
      `${context}.previous_anchor_hash`,
    ),
...
export function validateEconomyObjectResponse<T extends JsonObject = JsonObject>(
  value: unknown,
  context: string,
): EconomyObjectResponse<T> {
  const record = requireRecord(value, context);
  return {
    object: assertJsonObject(record.object, `${context}.object`) as T,
