...
export function assertUsageMetrics(usage: ProviderUsageMetrics): void {
  ...
  if (usage.total_tokens < usage.input_tokens + usage.output_tokens) {
    throw new LynkValidationError(
      "LYNK usage total_tokens must be at least input_tokens plus output_tokens",
    );
  }
...
function assertNonNegativeSafeInteger(name: string, value: number): void {
  if (!Number.isSafeInteger(value) || value < 0) {
    throw new LynkValidationError(
      `LYNK usage field ${name} must be a non-negative safe integer`,
    );
  }
}

export function buildLlmUsageEvidence(
  ...
  if (config.storageMode === "receipt_minimized" && encryptedPayloadRefs.length !== 0) {
    throw new LynkValidationError(
      "receipt_minimized LYNK evidence must not include encrypted payload refs",
    );
  }
  if (config.storageMode === "external_payload_ref" && encryptedPayloadRefs.length === 0) {
    throw new LynkValidationError(
      "external_payload_ref LYNK evidence requires encrypted payload refs",
    );
  }
  ...
  return {
    ...
    completion_hash:
      context.responsePayload === undefined ? undefined : hashProviderPayload(context.responsePayload),
    tool_call_hash:
      context.toolCallPayload === undefined ? undefined : hashProviderPayload(context.toolCallPayload),
    tool_result_hash:
      context.toolResultPayload === undefined
        ? undefined
...
export async function buildLlmUsageReceiptIntent(
  config: LlmProxyConfig,
  context: UsageContext,
): Promise<ReceiptIntent> {
  const evidence = buildLlmUsageEvidence(config, context);
  const envelope: LlmUsageEvidenceEnvelope = {
    schema_version: AVC_SCHEMA_VERSION,
  ...
  const adapterSignature =
    typeof config.adapterSignature === "function"
...
export async function maybeStoreExternalPayloads(
  config: LlmProxyConfig,
  ...
  if (!config.kms || !config.objectStore) {
    throw new LynkConfigurationError(
      "external_payload_ref requires both a customer KMS and object store",
    );
  }
  const refs: EncryptedPayloadRef[] = [];
...
export function assertNoForbiddenReceiptMaterial(value: unknown): void {
...
function assertRequiredConfig(config: LlmProxyConfig): void {
